...
// ─────────────────────────────────────────────────────────────────────────────
const cellStyles = {};
const getCellStyle  = (r, c) => cellStyles[`${r}_${c}`] || {};
const setCellStyle  = (r, c, patch) => { cellStyles[`${r}_${c}`] = { ...getCellStyle(r,c), ...patch }; };
const clearCellStyles = () => { Object.keys(cellStyles).forEach((k) => delete cellStyles[k]); };
...
const cpSt = {
  panel: { position:'absolute', top:'100%', left:0, zIndex:300, background:'#fff', border:'1px solid #d0d0d0', borderRadius:4, boxShadow:'0 4px 12px rgba(0,0,0,0.18)', padding:8, width:196 },
...
const bpSt = {
  panel:    { position:'absolute', top:'100%', left:0, zIndex:300, background:'#fff', border:'1px solid #d0d0d0', borderRadius:4, boxShadow:'0 4px 12px rgba(0,0,0,0.18)', padding:10, width:224 },
...
function SpreadsheetEditor({ fileId }) {
  ...
  const { otherCursors, broadcastCellChange, broadcastCursorMove, setCollabSheet, onlineUsers } = useCollabContext();
  ...
  useEffect(() => {
    ...
    setCollabSheet(currentSheetId, (row, col, value) => {
      ...
      if (hot) hot.setDataAtCell(row, col, value ?? '', 'remote');
    });
  ...
  const toggleFmt = useCallback((key) => {
    const next = !fmt[key];
    getSelCells().forEach(([r,c]) => setCellStyle(r, c, { [key]: next }));
    setFmt((p) => ({ ...p, [key]: next }));
    rerender();
  }, [fmt, getSelCells, rerender]);

  const applyAlign = useCallback((align) => {
    getSelCells().forEach(([r,c]) => setCellStyle(r, c, { align }));
    setFmt((p) => ({ ...p, align }));
    rerender();
  }, [getSelCells, rerender]);

  const applyFontColor = useCallback((color) => {
    setFClr(color);
    getSelCells().forEach(([r,c]) => setCellStyle(r, c, { color }));
    rerender();
  }, [getSelCells, rerender]);

  const applyBgColor = useCallback((color) => {
    setBClr(color);
    getSelCells().forEach(([r,c]) => setCellStyle(r, c, { background: color }));
    rerender();
  }, [getSelCells, rerender]);
  ...
  const handleAfterChange = useCallback((changes, source) => {
    ...
    // BUG FIX 2: 更新保存状态为"保存中"
    setSyncStatus('syncing');
    for (const [row, col, , newVal] of changes) {
      const c=Number(col), v=newVal??'';
      saveCell(row, c, v);
      broadcastCellChange(row, c, v);
    }
    // 模拟保存完成
    setTimeout(() => {
...
const st = {
  ...
  toolbar:{ display:'flex', alignItems:'center', height:36, padding:'0 8px', gap:2, background:'#f8f8f8', borderBottom:'1px solid #e0e0e0', flexShrink:0, overflowX:'auto' },
