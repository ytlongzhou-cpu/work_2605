...
const { upsertCellsCore, upsertStyleCore } = require('../cells/cellsController');
...
function initSocketHandler(io) {
  ...
  // ── 全局在线用户表（socketId → userInfo），用于大厅广播 ──
  const globalOnline = new Map(); // socketId → { userId, displayName, color }
  ...
  io.on('connection', (socket) => {
    ...
    // 加入全局在线列表并广播给所有人
    globalOnline.set(socket.id, { userId: user.id, displayName: user.displayName, color: color.cursor });
    // 同时单独推给自己，确保自己也能立刻看到最新在线列表（避免竞态）
    const globalList = Array.from(globalOnline.values());
    ...
    socket.on('style:update', async (payload) => {
      ...
      try {
        // 持久化样式到数据库（style 字段，value 保持不变用 undefined）
        const styleCells = cells.map(c => ({
          row:   c.row,
          col:   c.col,
          value: undefined, // 不更新 value
          style: c.style,
        }));
        await upsertStyleCore(sheetId, styleCells, user.id);
        ...
        console.error('[socketHandler] style:update error:', err);
        socket.emit('error', { message: '样式保存失败', code: 500 });
      }
    });
