...
exports.getDirectories = async (req, res) => {
    try {
        ...
        let result;
        if (isAdmin) {
            // BUG FIX：管理员需要获取所有目录（含归档），前端根据 is_archived 字段分区展示
            result = await pool.request()
                .query('SELECT * FROM directories ORDER BY parent_id, name');
        } else {
...
exports.getFileById = async (req, res) => {
    try {
        ...
        if (isNaN(fileId)) return res.status(400).json({ success: false, error: '无效的 fileId', code: 400 });
        const pool = await getPool();
        const result = await pool.request()
            .input('id', sql.Int, fileId)
            .query('SELECT id, name, directory_id, is_archived FROM files WHERE id = @id');
        if (!result.recordset[0]) return res.status(404).json({ success: false, error: '文件不存在', code: 404 });
...
exports.getFiles = async (req, res) => {
    try {
        ...
        // BUG FIX：支持 ?include_archived=1 参数，用于加载包含归档文件的完整列表
        const includeArchived = req.query.include_archived === '1';
        const pool = await getPool();
        const query = includeArchived
            ? 'SELECT * FROM files WHERE directory_id=@dirId ORDER BY name'
            : 'SELECT * FROM files WHERE directory_id=@dirId AND is_archived=0 ORDER BY name';
        const result = await pool.request()
