...
// BUG FIX：行列结构变更接口（删/插行列持久化到 DB）
router.delete('/cells/:sheetId/rows', authenticateToken, checkFilePermission('write'), deleteRows);
router.delete('/cells/:sheetId/cols', authenticateToken, checkFilePermission('write'), deleteCols);
router.post('/cells/:sheetId/rows',   authenticateToken, checkFilePermission('write'), insertRows);
router.post('/cells/:sheetId/cols',   authenticateToken, checkFilePermission('write'), insertCols);
