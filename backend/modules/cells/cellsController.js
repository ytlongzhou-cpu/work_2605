...
async function deleteRows(req, res) {
  ...
  try {
    ...
    await transaction.request()
      ...
      .query('UPDATE cells SET row_index = row_index - @shift WHERE sheet_id=@sid AND row_index >= @rStart');
    await transaction.commit();
    res.json({ success: true });
  } catch (err) {
    try { await transaction.rollback(); } catch (_) {}
...
async function deleteCols(req, res) {
  ...
  try {
    ...
    await transaction.request()
      ...
      .input('shift',  sql.Int, amount)
      .query('UPDATE cells SET col_index = col_index - @shift WHERE sheet_id=@sid AND col_index >= @cStart');
    await transaction.commit();
    res.json({ success: true });
  } catch (err) {
    try { await transaction.rollback(); } catch (_) {}
