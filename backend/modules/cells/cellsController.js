...
function setCollaborationIo(io) {
  collaborationIo = io;
}

function broadcastStructureChange(eventName, sheetId, userId, index, amount) {
  if (!collaborationIo) return;
  collaborationIo.to(String(sheetId)).emit(eventName, {
    userId,
    sheetId,
    index,
    amount,
...
module.exports = {
  ...
  upsertCells,
  saveCellStyles,
  // BUG FIX：必须在此处直接导出，Object.assign 追加方式在 socketHandler
  // 解构导入时拿不到（require() 返回的是 module.exports 的初始引用快照）
  upsertCellsCore,
  upsertStyleCore,
  // HTTP 接口：行列结构变更
  deleteRows,
  ...
  insertCols,
  setCollaborationIo,
  // WebSocket 直接调用的 Core 版本（无 req/res，供 socketHandler 使用）
  deleteRowsCore,
  deleteColsCore,
  insertRowsCore,
  insertColsCore,
...
// ════════════════════════════════════════════════════════════════════
// BUG FIX：行列结构变更接口
// 删/插行列时需要同步更新数据库中的 row_index / col_index
// ════════════════════════════════════════════════════════════════════

/**
 * DELETE /api/cells/:sheetId/rows  body: { index, amount }
 * 删除指定行：先删该行数据，再把后续行的 row_index 减 amount
 */
async function deleteRows(req, res) {
  const sheetId = parseInt(req.params.sheetId, 10);
  const index  = parseInt(req.body.index,  10);
  const amount = parseInt(req.body.amount, 10) || 1;
  if (!sheetId || !Number.isFinite(index)) {
    return res.status(400).json({ success: false, error: '参数错误', code: 400 });
  }
  const pool = await getPool();
  const transaction = pool.transaction();
  try {
    ...
    // 1. 删除目标行数据
    await transaction.request()
      .input('sid',   sql.Int, sheetId)
      .input('rFrom', sql.Int, index)
      .input('rTo',   sql.Int, index + amount - 1)
      .query('DELETE FROM cells WHERE sheet_id=@sid AND row_index BETWEEN @rFrom AND @rTo');
    // 2. 后续行 row_index 上移
    await transaction.request()
      .input('sid',    sql.Int, sheetId)
      .input('rStart', sql.Int, index + amount)
      .input('shift',  sql.Int, amount)
      .input('tmpShift', sql.Int, 100000000)
      .query(`
        UPDATE cells SET row_index = row_index + @tmpShift WHERE sheet_id=@sid AND row_index >= @rStart;
        UPDATE cells SET row_index = row_index - @tmpShift - @shift WHERE sheet_id=@sid AND row_index >= @rStart + @tmpShift;
      `);
    await transaction.commit();
    broadcastStructureChange('row:deleted', sheetId, req.user.id, index, amount);
    res.json({ success: true });
  } catch (err) {
    try { await transaction.rollback(); } catch (_) {}
    console.error('[deleteRows]', err);
    res.status(500).json({ success: false, error: '删除行失败', code: 500 });
  }
}

/**
 * DELETE /api/cells/:sheetId/cols  body: { index, amount }
 * 删除指定列：先删该列数据，再把后续列的 col_index 减 amount
 */
async function deleteCols(req, res) {
  const sheetId = parseInt(req.params.sheetId, 10);
  const index  = parseInt(req.body.index,  10);
  const amount = parseInt(req.body.amount, 10) || 1;
  if (!sheetId || !Number.isFinite(index)) {
  ...
  try {
    ...
    await transaction.request()
      .input('sid',   sql.Int, sheetId)
      .input('cFrom', sql.Int, index)
      .input('cTo',   sql.Int, index + amount - 1)
      .query('DELETE FROM cells WHERE sheet_id=@sid AND col_index BETWEEN @cFrom AND @cTo');
    await transaction.request()
      .input('sid',    sql.Int, sheetId)
      .input('cStart', sql.Int, index + amount)
      .input('shift',  sql.Int, amount)
      .input('tmpShift', sql.Int, 100000000)
      .query(`
        UPDATE cells SET col_index = col_index + @tmpShift WHERE sheet_id=@sid AND col_index >= @cStart;
        UPDATE cells SET col_index = col_index - @tmpShift - @shift WHERE sheet_id=@sid AND col_index >= @cStart + @tmpShift;
      `);
    await transaction.commit();
    broadcastStructureChange('col:deleted', sheetId, req.user.id, index, amount);
    res.json({ success: true });
  } catch (err) {
    try { await transaction.rollback(); } catch (_) {}
    console.error('[deleteCols]', err);
    res.status(500).json({ success: false, error: '删除列失败', code: 500 });
  }
}

/**
 * POST /api/cells/:sheetId/rows  body: { index, amount }
 * 插入行：把 >= index 的行的 row_index 加 amount
 */
async function insertRows(req, res) {
  ...
  try {
    ...
    await transaction.request()
      ...
      `);
    await transaction.commit();
    broadcastStructureChange('row:inserted', sheetId, req.user.id, index, amount);
    res.json({ success: true });
  } catch (err) {
    try { await transaction.rollback(); } catch (_) {}
    console.error('[insertRows]', err);
    res.status(500).json({ success: false, error: '插入行失败', code: 500 });
  }
}

/**
 * POST /api/cells/:sheetId/cols  body: { index, amount }
 * 插入列：把 >= index 的列的 col_index 加 amount
 */
async function insertCols(req, res) {
  const sheetId = parseInt(req.params.sheetId, 10);
  const index  = parseInt(req.body.index,  10);
  const amount = parseInt(req.body.amount, 10) || 1;
  if (!sheetId || !Number.isFinite(index)) {
  ...
  try {
    ...
    await transaction.request()
      ...
      `);
    await transaction.commit();
    broadcastStructureChange('col:inserted', sheetId, req.user.id, index, amount);
    res.json({ success: true });
  } catch (err) {
    try { await transaction.rollback(); } catch (_) {}
    console.error('[insertCols]', err);
    res.status(500).json({ success: false, error: '插入列失败', code: 500 });
  }
}


// ════════════════════════════════════════════════════════════════════
// WebSocket Core 函数：行列结构变更（不依赖 req/res，供 socketHandler 直接调用）
// ════════════════════════════════════════════════════════════════════

/**
 * 删除行（Core版，供 socketHandler 调用）
 * @param {number} sheetId
 * @param {number} index   起始行索引（0-based）
 * @param {number} amount  删除行数，默认 1
 */
async function deleteRowsCore(sheetId, index, amount = 1) {
  const pool = await getPool();
  const transaction = pool.transaction();
  try {
    await transaction.begin();
...
/**
 * 删除列（Core版，供 socketHandler 调用）
 * @param {number} sheetId
 * @param {number} index   起始列索引（0-based）
 * @param {number} amount  删除列数，默认 1
 */
async function deleteColsCore(sheetId, index, amount = 1) {
  const pool = await getPool();
  const transaction = pool.transaction();
  try {
    await transaction.begin();
...
/**
 * 插入行（Core版，供 socketHandler 调用）
 * @param {number} sheetId
 * @param {number} index   插入位置（0-based）
 * @param {number} amount  插入行数，默认 1
 */
async function insertRowsCore(sheetId, index, amount = 1) {
  const pool = await getPool();
  const transaction = pool.transaction();
  try {
    await transaction.begin();
...
/**
 * 插入列（Core版，供 socketHandler 调用）
 * @param {number} sheetId
 * @param {number} index   插入位置（0-based）
 * @param {number} amount  插入列数，默认 1
 */
async function insertColsCore(sheetId, index, amount = 1) {
  const pool = await getPool();
  const transaction = pool.transaction();
  try {
    await transaction.begin();
