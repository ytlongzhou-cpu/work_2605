...
async function upsertCells(req, res) {
  ...
  try {
    ...
    for (const cell of cells) {
      // value 为 null / undefined / 空字符串统一存为 NULL
      const cellValue =
      ...
      await transaction
        ...
        .query(`
          ...
          WHEN NOT MATCHED THEN
            INSERT (sheet_id, row_index, col_index, value, updated_by)
            VALUES (source.sheet_id, source.row_index, source.col_index,
                    source.value,    source.updated_by);
        `);
    }
...
// 覆盖原有 exports，新增 upsertCellsCore
Object.assign(module.exports, { upsertCellsCore });
